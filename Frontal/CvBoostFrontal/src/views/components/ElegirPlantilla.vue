...
const plantillas = computed(() => [
  { id: 'europass',    label: 'Europass',    desc: langStore.t.tpl_euro_desc },
  { id: 'minimalista', label: 'Minimalista', desc: langStore.t.tpl_mini_desc },
  { id: 'moderna',     label: 'Moderna',     desc: langStore.t.tpl_mod_desc },
])
...
async function guardarCV() {
  ...
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/api/cv`, {
      ...
      body: JSON.stringify({ ...store.formulario, plantilla: plantillaSeleccionada.value }),
    })
...
<template>
  <div v-if="store.pagina === 9">
    <h1>{{ langStore.t.tpl_title }}</h1>
    <h2>{{ store.ponerFotoParaDialog ? langStore.t.tpl_with_photo : langStore.t.tpl_no_photo }}</h2>
    ...
    <div class="nav-buttons" style="margin-top: 20px;">
      <button @click="store.anteriorPagina()" type="button" class="secondary">{{ langStore.t.tpl_back }}</button>
      <button @click="guardarCV()" type="button">{{ langStore.t.tpl_generate }}</button>
    </div>
  </div>
  ...
  <Teleport to="body">
    <div v-if="cargando" class="cv-overlay">
      ...
      <p class="cv-text">{{ langStore.t.tpl_generating }}</p>
    </div>
  </Teleport>
</template>
