    ...
    <div class="cv">
      ...
      <aside class="sidebar">
        <div class="sb-photo-wrap">
          <img v-if="cvData.foto_base64" :src="cvData.foto_base64" class="sb-photo" alt="Foto" />
          <div v-else class="sb-photo-placeholder">
            <svg viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="30" cy="22" r="12" fill="#3d4a5c"/>
              <path d="M6 56c0-13.255 10.745-24 24-24s24 10.745 24 24" fill="#3d4a5c"/>
            </svg>
          </div>
        </div>

        <div class="sb-nombre">
          <span class="sb-nombre-first">{{ cvData.datosPersonales?.nombre }}</span>
          <span class="sb-nombre-last">{{ cvData.datosPersonales?.apellido }}</span>
        </div>

        <section class="sb-section">
          <h3 class="sb-section-title">Contacto</h3>
          <div class="sb-item"><span class="sb-icon">✉</span><span>{{ cvData.datosPersonales?.email }}</span></div>
          <div class="sb-item"><span class="sb-icon">📱</span><span>{{ cvData.datosPersonales?.telefono }}</span></div>
          <div class="sb-item"><span class="sb-icon">📍</span><span>{{ cvData.datosPersonales?.direccion }}, {{ cvData.datosPersonales?.localidad }}</span></div>
          <div v-if="cvData.datosPersonales?.permiso_conducir" class="sb-item"><span class="sb-icon">🚗</span><span>Permiso B</span></div>
        </section>
        ...
        <section v-if="cvData.idiomas?.length" class="sb-section">
          ...
          <div v-for="(lang, i) in cvData.idiomas" :key="i" class="sb-lang">
            <div class="sb-lang-header">
              <span>{{ lang.nombre || lang.idioma }}</span>
        ...
        <section v-if="cvData.skills?.length" class="sb-section">
          ...
          <div class="sb-skills">
            <span v-for="(skill, i) in cvData.skills" :key="i" class="sb-skill">{{ skill }}</span>
          </div>
        </section>
      </aside>
      ...
      <main class="main">
        <header class="main-header">
          <h1 class="main-name">{{ cvData.datosPersonales?.nombre }} <span>{{ cvData.datosPersonales?.apellido }}</span></h1>
          <p v-if="cvData.datosPersonales?.profesion" class="main-title">{{ cvData.datosPersonales.profesion }}</p>
        ...
        <section v-if="cvData.descripcion" class="main-section">
          ...
          <p class="main-descripcion">{{ cvData.descripcion }}</p>
        </section>
          ...
          <div v-for="(exp, i) in cvData.experiencia" :key="i" class="main-entry">
            ...
            <div class="main-entry-content">
              <div class="main-entry-top">
                <strong class="main-entry-role">{{ exp.puesto || exp.cargo }}</strong>
                <span class="main-entry-badge">{{ exp.mesInicio || '' }} {{ exp.anioInicio }} — {{ exp.actualidad ? 'Actualidad' : ((exp.mesFin || '') + ' ' + (exp.anioFin || '')) }}</span>
              </div>
              <div class="main-entry-company">{{ exp.empresa }}</div>
            </div>
          </div>
        </section>
        ...
        <section v-if="cvData.educacion?.length || cvData.certificaciones?.length" class="main-section">
          ...
          <div v-for="(edu, i) in cvData.educacion" :key="'e'+i" class="main-entry">
            ...
            <div class="main-entry-content">
              <div class="main-entry-top">
                <strong class="main-entry-role">{{ edu.titulo }}</strong>
                <span class="main-entry-badge">{{ edu.anioInicio }} — {{ edu.actualidad ? 'Actualidad' : edu.anioFin }}</span>
              </div>
              <div class="main-entry-company">{{ edu.institucion }}</div>
            </div>
          </div>
          <div v-for="(cert, i) in cvData.certificaciones" :key="'c'+i" class="main-entry">
            ...
            <div class="main-entry-content">
              <div class="main-entry-top">
                <strong class="main-entry-role">{{ cert.certificacion }}</strong>
                <span class="main-entry-badge">{{ cert.mes }} {{ cert.anio }}</span>
              </div>
            </div>
          </div>
        </section>
...
.main-name { font-size: 22pt; font-weight: 300; color: #111; line-height: 1.1; letter-spacing: -0.5px; }
.main-name span { font-weight: 700; }
...
.main-entry-badge { font-size: 7pt; background: #f4f4f4; color: #666; border-radius: 20px; padding: 2px 8px; white-space: nowrap; font-weight: 500; flex-shrink: 0; margin-top: 2px; }
